...
class SettingsHandler:
    ...
    def update(self, module_name: str, default_settings, guild_id: int):
        """
        Used to populate new configuration variables if the current
        configuration does not have said variables.

        Parameters
        ----------
        module_name: :class:`str`
            The name of the module to check.
        default_settings: :class:`dict`
            The default settings of a module.
        guild_id: :class:`int`
            The id of the guild, to get that guilds settings and check the
    ...
    def get_key_map(self, settings, values: bool = False) -> Union[list[str], dict]:
        def walk(key_list: Union[list[str], dict], branch: dict, full_branch_key: str):
            walk_list = {} if values is True else []
            for key, value in branch.items():
                if type(branch[key]) is dict:
                    walk(key_list, branch[key], f"{full_branch_key}.{key}")
                else:
                    if type(walk_list) is dict:
                        walk_list[f"{full_branch_key}.{key}.".lower()] = value
                    else:
                        walk_list.append(f"{full_branch_key}.{key}")

            if type(walk_list) is dict:
                key_list.update(walk_list)
            else:
                key_list.extend(walk_list)
            return key_list

        key_map = {} if values is True else []
        for key, value in settings.items():
            if type(settings[key]) is dict:
                key_map = walk(key_map, settings[key], key)
            else:
                if values:
                    key_map[key.lower()] = value
                else:
                    key_map.append(key.lower())
        return key_map
...
async def get_member_from_string(
    ctx: Optional[Context], string: str, *, guild: discord.Guild = None
    ...
    for part in string.split():
        member_match = await member_func(
        ...
        if member_match is None:
            ...
            # if member is None, but previous result is a list, return Normal get_member call and allow user to choose member
                ...
                return (
                    await member_func(ctx, f"{member_name}".strip()),
    ...
    for mention in mentions:
        ...
        if mention_object:
            string = string.replace(
                f"{mention[0]}", f"{mention_type[0]}{mention_object.name}"
            )
...
def embed_message_to_text(embed):
    ...
    text = f"{title}" f"{description}" f"{fields}"
