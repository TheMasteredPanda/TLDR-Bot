...
import config
import discord
from discord.ext.commands import (ChannelNotFound, Cog, Context,
                                  TextChannelConverter, command, group)
from modules import commands, database, embed_maker, reaction_menus
from modules.utils import (ParseArgs, embed_message_to_text, get_custom_emote,
                           get_guild_role, get_member, get_text_channel,
                           replace_mentions)
...
class Admin(Cog):
    ...
    async def slack_channel(self, ctx: Context):
        ...
        for team in self.bot.slack_bridge.teams:
            unbridged_channels = " | ".join(
                f"`{channel.id}`"
                for channel in team.channels
                if not channel.discord_channel
            )
            bridged_channels = "\n".join(
                f"`{channel.id}` - [<#{channel.discord_channel.id}>]"
                for channel in team.channels
                if channel.discord_channel
            )
            unbridge_value += (
                (f"\n**{team.name}:** " + unbridged_channels)
                if unbridged_channels
                else ""
            )
            bridged_value += (
    ...
    async def slack_channel_info(self, ctx: Context, channel_id: str = None):
        ...
        if not slack_channel:
            return await embed_maker.error(
                ctx, f"Unable to find slack channel by the id [{channel_id}]"
            )

        description = (
            f"**Slack ID:** {slack_channel.id}\n"
        ...
        if slack_channel.discord_channel:
            description += (
                f"**Discord ID:** {slack_channel.discord_channel.id}\n"
    ...
    async def slack_channel_bridge(
        self, ctx: Context, slack_channel_id: str = None, discord_channel: str = None
    ):
        ...
        if slack_channel is None:
            return await embed_maker.error(
                ctx, f"Unable to find slack channel via id [{slack_channel_id}]"
            )
        ...
        if slack_channel.discord_channel:
            return await embed_maker.error(
                ctx,
                f"Slack channel [{slack_channel_id}] is already assigned to discord channel [{slack_channel.discord_channel.mention}]",
            )
        ...
        if taken:
            return await embed_maker.error(
                ctx,
                f"Discord channel [{discord_channel.mention}] is already assigned to Slack channel [{taken.id}]",
            )
    ...
    async def slack_member(self, ctx: Context):
        ...
        alias = (
            lambda slack_member: f"<@{slack_member.discord_member.id}>"
            if slack_member.discord_member
            else slack_member.name
        )
        unaliased_value = ""
        aliased_value = ""
        for team in self.bot.slack_bridge.teams:
            unaliased_members = " | ".join(
                f"`{member.id}`"
                for member in team.members
                if not member.discord_member and not member.discord_name
            )
            aliased_members = "\n".join(
                f"`{member.id}` - [{alias(member)}]"
                for member in team.members
                if member.discord_member or member.discord_name
            )
            unaliased_value += (
        ...
        if aliased_value:
            embed.add_field(
                name=">Member with aliases", value=aliased_value, inline=False
            )
    ...
    async def slack_member_info(self, ctx: Context, member_id: str = None):
        ...
        if not slack_member:
            return await embed_maker.error(
                ctx, f"Unable to find slack member by the id [{member_id}]"
            )

        description = (
            f"**Slack ID:** {slack_member.id}\n"
            f"**Slack Name:** {slack_member.slack_name}\n"
        )

        if slack_member.discord_member or slack_member.discord_name:
            description += (
                f'**Discord ID:** {slack_member.discord_member.id if slack_member.discord_member else "None"}\n'
        ...
        return await embed_maker.message(
            ctx, author={"name": slack_member.id}, description=description, send=True
        )
    ...
    async def slack_member_alias(
        self,
        ctx: Context,
        slack_member_id: str = None,
        *,
        discord_member_identifier: str = None,
    ):
        ...
        if slack_member is None:
            return await embed_maker.error(
                ctx, f"Unable to find slack member via id [{slack_member_id}]"
            )
        ...
        if not ctx.message.mentions and not discord_member_identifier.isdigit():
            ...
            if taken:
                return await embed_maker.error(
                    ctx,
                    f"Member [{discord_member.mention}] is already linked with [{taken.id}]",
                )
    ...
    async def clearance_refresh(self, ctx: Context):
        ...
        return await embed_maker.message(
            ctx,
            description="Clearance data has been refreshed",
            send=True,
            colour="green",
        )
    ...
    async def clearance_groups(self, ctx: Context):
        ...
        for group_name, group_roles in groups.items():
            embed.add_field(
                name=f">{group_name}",
                value=" | ".join(f"`{role}`" for role in group_roles),
                inline=False,
            )
    ...
    async def clearance_roles(self, ctx: Context):
        ...
        await embed_maker.message(
            ctx, description=description, author={"name": "Roles"}, send=True
        )
    ...
    @command(
        ...
        examples=["emote_roles :TldrNewsUK:", "emote_roles Mayor"],
        cls=commands.Command,
    )
    async def emote_roles(self, ctx, user_input: str = None):
        ...
        if emote:
            if emote.roles:
                ...
                return await embed_maker.message(
                    ctx, description="This emote is available to everyone", send=True
                )
        elif role:
            emotes = []
    ...
    async def emote_role(
        self, ctx: Context, action: str = None, *, args: Union[ParseArgs, dict] = None
    ):
        ...
        emote_list = [
            *filter(
                ...
                [get_custom_emote(ctx, emote) for emote in emotes.split(" ")],
            )
        ]
        if not emote_list:
            return await embed_maker.command_error(ctx, "[emotes]")
    ...
    async def archive_channel(self, ctx: Context):
        ...
        channel_history_string = await self.history_into_string(
            ctx.guild, channel_history
        )

        if config.SERVICE_ACCOUNT_FILE:
            try:
                file_url = self.bot.google_drive.upload(
                    channel_history_string,
                    f"{ctx.channel.name} - archive.txt",
                    "channel archives",
                )
            ...
            if error:
                await embed_maker.error(
                    ctx, "Error uploading archive file to google drive"
                )
            else:
                return await embed_maker.message(
                    ctx,
                    description=f"File has been been uploaded to google drive: {file_url}",
                    send=True,
                )

        # send file containing archive
        return await ctx.send(
            file=discord.File(StringIO(channel_history_string), filename="archive.txt")
        )
        ...
        *,
        ...
        if custom_commands:
            ...
            for i, command in enumerate(
                custom_commands[page_size_limit * (page - 1) : page_size_limit * page]
            ):
                if i == 10:
                    break
    ...
    async def customcommands(self, ctx: Context, index: Union[int, str] = None):
        if not ctx.subcommand_passed:
            ...
            if index is None:
                ...
                command = {i: c for i, c in enumerate(custom_commands)}.get(
                    index - 1, None
                )
                if not command:
    ...
    async def validate_custom_command_args(
        self, ctx: Context, args: dict, *, edit: bool = False
    ):
        if not args["name"] and not edit:
            return await embed_maker.error(
                ctx, "A name needs to be given to the custom command."
            )

        member_clearance = self.bot.command_system.member_clearance(ctx.author)
        # check for existing custom command with the same name
        existing = db.custom_commands.find_one(
            {"guild_id": ctx.guild.id, "name": args["name"]}
        )
        if existing:
            return await embed_maker.error(
                ctx, f"A custom command with the name `{args['name']}` already exists"
            )
        ...
        if args["python"] and "Developers" not in member_clearance["groups"]:
            return await embed_maker.error(
                ctx, f'Only devs can {"add" if not edit else "edit"} the python arg'
            )

        # validate clearances
        clearance_groups = args["clearance-groups"] = [
            g.strip() for g in args["clearance-groups"].split(",")
        ]
        clearance_roles = args["clearance-roles"] = [
            r.strip() for r in args["clearance-roles"].split(",")
        ]
        clearance_users = args["clearance-users"] = [
            u.strip() for u in args["clearance-users"].split(",")
        ]
        ...
        if invalid_groups:
            return await embed_maker.error(
                ctx,
                f'Groups {", ".join(f"`{g}`" for g in invalid_groups)} are not in the clearance spreadsheet.',
            )
        ...
        if invalid_roles:
            return await embed_maker.error(
                ctx,
                f'Roles {", ".join(f"`{r}`" for r in invalid_roles)} are not in the clearance spreadsheet.',
            )

        for user_id in clearance_users:
            if not user_id.isdigit() or not self.bot.get_user(int(user_id)):
                return await embed_maker.error(
                    ctx, f"`{user_id}` is not a valid user id"
                )
        ...
        if args["response-channel"]:
            try:
                response_channel = await TextChannelConverter().convert(
                    ctx, args["response-channel"]
                )
                args["response-channel"] = response_channel.id
            except ChannelNotFound:
                return await embed_maker.error(
                    ctx, f'Invalid response-channel arg: `{args["response-channel"]}`'
                )
        ...
        if args["command-channels"]:
            ...
            for i, channel in enumerate(args["command-channels"]):
                try:
                    ...
                    return await embed_maker.error(
                        ctx, f"Invalid command-channel arg: `{channel}`"
                    )
        ...
        if args["response"]:
            ...
            args["response"] = re.findall(
                r"^(?:```\n)?(.*)(?:(?:\n```)$)?", args["response"], re.MULTILINE
            )[0]
        ...
        if args["response"]:
            ...
            for command_name in command_matches:
                ...
                if not command:
                    return await embed_maker.error(
                        ctx, f"Invalid command: `>{command_name}`"
                    )
    ...
    @customcommands.command(
        ...
        command_args=[
            (
                ("--name", "-n", str),
                "The name given to the command, matched with regex.",
            ),
            (
                ("--response", "-r", str),
                r"Response that will be sent when custom command is called, If you want to do multiline response, please put response between \`\`\`",
            ),
            (
                ("--clearance-groups", "-cg", str),
                f"[Optional] Restrict who can use the command to groups in the clearance spreadsheet, values need to be seperated via commas.",
            ),
            (
                ("--clearance-roles", "-cr", str),
                f"[Optional] Restrict who can use the command to roles in the clearance spreadsheet, values need to be seperated via commas.",
            ),
            (
                ("--clearance-users", "-cu", str),
                "[Optional] Restrict who can use the command to user, values need to be user ids and seperated via commas.",
            ),
            (
                ("--response-channel", "-rc", str),
                "[Optional] When set, response will be sent in given channel instead of where custom command was called",
            ),
            (
                ("--command-channels", "-cc", str),
                "[Optional] Restrict custom command to give channel(s)",
            ),
            (
                ("--reactions", "-rs", str),
                "[Optional] Reactions that will be added to response, emotes only ofc.",
            ),
            (
                ("--python", "-p", str),
                r"[Dev only] Run a python script when custom command is called, please put code between \`\`\`",
            ),
        ],
        cls=commands.Command,
    )
    ...
    async def customcommands_edit(self, ctx: Context, *, args: ParseArgs = None):
        ...
        existing = db.custom_commands.find_one(
            {"guild_id": ctx.guild.id, "name": old_command_name}
        )
        if not existing:
            return await embed_maker.error(
                ctx, f"A command with the name `{old_command_name}` doesn't exist"
            )
        ...
        db.custom_commands.update_one(
            {"guild_id": ctx.guild.id, "name": old_command_name}, {"$set": args}
        )
    ...
    async def customcommands_remove(self, ctx: Context, index: Union[int, str] = None):
        ...
        db.custom_commands.delete_one(
            {"guild_id": ctx.guild.id, "name": command["name"]}
        )
    ...
    async def customcommands_variables(self, ctx: Context):
        with open("static/custom_commands_variables.txt", "r") as file:
            return await embed_maker.message(
                ctx,
                description=f"```{file.read()}```",
                author={"name": "Custom Command Variables"},
                send=True,
            )
