...
class Mod(Cog):
    ...
    async def reprimand_poll(self, ctx: Context, *, args: Union[ParseArgs, str] = None):
        ...
        print(cg_ids)
        reprimand = (
            await self._reprimand_module.get_reprimand_manager().create_reprimand(
                result[0], cg_ids
            )
        )
    ...
    async def config_notification_add(
        ...
        notifications_config_copy = self._reprimand_module.get_settings()[
            "notifications"
        ]
        ...
        self._reprimand_module.set_setting("notifications", notifications_config_copy)
        return await embed_maker.message(
            ctx,
    ...
    async def config_notification_remove(self, ctx: Context, interval: str = ""):
        notification_config_copy = copy.deepcopy(
            self._reprimand_module.get_settings()["notifications"]
        )
        ...
        self._reprimand_module.set_setting("notifications", notification_config_copy)
        return await embed_maker.message(
            ctx,
    ...
    async def punishments_view(self, ctx: Context):
        ...
        for key, punishment in punishments_config_copy.items():
            await embed_maker.message(
                ctx,
                description=f"```{json_util.dumps(punishment, indent=4)}```",
                title=f"Punishment: {key}",
                send=True,
            )
    ...
    async def punishment_add(self, ctx: Context, *, args: Union[ParseArgs, str] = None):
        ...
        for punishment_id in punishments:
            ...
            if punishment is not None:
                print(args)
                if "duration" in punishment.keys():
                    if (
        ...
        self._reprimand_module.add_punishment(
            args["id"].lower(),
            PunishmentType.from_name(args["type"]),
            args["duration"],
            args["name"],
            short_description,
            args["emoji"],
        ...
        await embed_maker.message(
            ctx,
            description=f"Created punishment {args['id'].lower()}.",
            title="Created.",
            send=True,
        )
    ...
    async def reprimand_time_thread_cmd(
        ...
        if (
            self._reprimand_module.get_reprimand_manager().is_reprimand_thread(
                channel.id
            )
            is False
        ):
        ...
        reprimand = self._reprimand_module.get_reprimand_manager().get_reprimand(
            channel.id
        )
